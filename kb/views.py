...
class InstructionCreate(CreateView):
    ...
    success_url = reverse_lazy('kb:user_manual_list') #TODO переход на manual этой инструкции
...
class InstructionList(FormMixin, ListView):
    queryset = kb.models.Instruction.objects.all()
...
class UserManualCreate(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            formset = kb.forms.ManualInstructionsFormSet(self.request.POST or None, self.request.FILES, instance=self.object)
            if formset.is_valid():
                formset.save()
    ...
    def get_context_data(self, **kwargs):
        context = super(UserManualCreate, self).get_context_data(**kwargs)
        formset = kb.forms.ManualInstructionsFormSet(self.request.GET or None, self.request.FILES or None)
...
class UserManualEdit(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(UserManualEdit, self).get_context_data(**kwargs)
        formset = kb.forms.ManualInstructionsFormSet(self.request.POST or None, self.request.FILES or None, instance=self.object)
        context['formset'] = formset
        return context
    ...
    def get(self, request, *args, **kwargs):
        ...
        for instruction in um.instructions.all():
            instruction_dict = {
                ...
                'file': instruction.file.url if instruction.file else ''
            }
...
class UserManualList(FormMixin, ListView):
...
#         form = self.get_form()
#         if form.is_valid():
#             kb_collection = create_kb_collection(form, request.FILES['json_file'])
